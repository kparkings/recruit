...
public class CountryExtractor implements JobSpecifcationFilterExtractor{
	...
	*/
	public void extractFilters(String documentText, CandidateExtractedFiltersBuilder filterBuilder) {
			...
			boolean includeNL = nl.stream().filter(place -> documentText.contains(" "+place+" ") || documentText.contains(" "+place+".")).count() > 0;
			boolean includeIE = ie.stream().filter(place -> documentText.contains(" "+place+" ") || documentText.contains(" "+place+".")).count() > 0;
			boolean includeBE = be.stream().filter(place -> documentText.contains(" "+place+" ") || documentText.contains(" "+place+".")).count() > 0;
